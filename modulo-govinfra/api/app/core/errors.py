...
logger = logging.getLogger("govinfra.error")
...
class RegraNegocio(AppError):
    """Impedimento previsto pelas regras do módulo (bloqueio, limite, agenda).

    `motivos` alimenta a interface, que precisa explicar ao atendente exatamente
    por que a operação não pôde ser concluída.
    """

    def __init__(self, message: str, motivos: list[dict] | None = None, code: str = "regra_negocio"):
        super().__init__(message, status.HTTP_422_UNPROCESSABLE_ENTITY, code, {"motivos": motivos or []})
