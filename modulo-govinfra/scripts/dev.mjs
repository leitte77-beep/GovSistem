#!/usr/bin/env node
/**
 * Sobe API e frontend do GovInfra em modo de desenvolvimento, sempre em portas
 * verificadas pelo resolvedor (nunca uma porta fixa sem checagem).
 *
 * Uso:
...
// 1. Portas — resolvidas antes de subir qualquer processo.
...
function encerrar() {
  for (const filho of processos) if (!filho.killed) filho.kill('SIGTERM');
  process.exit(0);
}
...
if (apenas !== 'web') {
  iniciar(
    'api',
    path.join(VENV_BIN, WINDOWS ? 'uvicorn.exe' : 'uvicorn'),
    ['app.main:app', '--host', '127.0.0.1', '--port', String(portas.api), '--reload'],
    {
      cwd: API_DIR,
...
if (apenas !== 'api' && fs.existsSync(path.join(WEB_DIR, 'package.json'))) {
  iniciar('web', 'npm', ['run', 'dev', '--', '--port', String(portas.frontend), '--strictPort'], {
    cwd: WEB_DIR,
    env: { ...process.env, VITE_GOVINFRA_API_URL: `http://127.0.0.1:${portas.api}` },
  });
}
