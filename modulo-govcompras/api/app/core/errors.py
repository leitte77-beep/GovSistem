...
Regra: o usuário final recebe uma mensagem clara em português; o detalhe
técnico vai só para o log, correlacionado pelo identificador da requisição.
"""
...
logger = logging.getLogger("govcompras.error")
...
class PermissionDenied(AppError):
    def __init__(self, message: str = "Você não possui permissão para esta operação."):
...
class Conflict(AppError):
    def __init__(self, message: str, code: str = "conflito", extra: dict | None = None):
        super().__init__(message, status.HTTP_409_CONFLICT, code, extra)


class PendenciasBloqueiam(AppError):
    """Avanço de etapa bloqueado por requisitos não satisfeitos (seção 109).

    `pendencias` alimenta a interface, que precisa listar exatamente o que
    falta em vez de um boolean genérico de "não é possível avançar".
    """

    def __init__(self, pendencias: list[dict]):
        super().__init__(
            "Não é possível avançar: existem pendências nesta etapa.",
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _app_error(request: Request, exc: AppError):
        return JSONResponse(
            ...
            content=_payload(request, exc.code, exc.message, exc.extra),
        )
