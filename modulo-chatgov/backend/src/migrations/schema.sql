...
-- Quando a ultima consulta de foto de perfil foi tentada. Sem isso o sistema
-- reconsultava o WhatsApp a cada mensagem de todo contato sem avatar (a maioria
-- responde not-authorized por privacidade), o que e trafego inutil e arrisca
-- rate-limit na conta do orgao.
ALTER TABLE contatos ADD COLUMN IF NOT EXISTS avatar_tentado_em TIMESTAMPTZ;
