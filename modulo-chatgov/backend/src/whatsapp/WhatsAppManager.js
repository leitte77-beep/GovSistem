...
export class WhatsAppManager extends EventEmitter {
  ...
  async restaurarSessoes() {
    const sessoes = await db.tx(async (t) => {
      await t.none('SET LOCAL row_security = off');
      return t.manyOrNone(
  ...
  async fetchProfilePicture(tenantId, jid) {
    try {
      ...
      // O catch silencioso fazia toda falha (privacidade, rate-limit, sessão)
      // virar "contato sem foto". O motivo importa: 401/403 é privacidade do
      // contato (definitivo), 429/408 é transitório e vale nova tentativa.
      const status = err?.output?.statusCode || err?.data || err?.status;
      console.warn(`[WA] profilePictureUrl falhou jid=${jid} status=${status ?? '?'} msg=${err?.message}`);
      return null;
    }
  }
