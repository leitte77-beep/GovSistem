...
import { ShieldCheck, LogOut, Sun, Moon, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
import { T } from '../theme';
...
function BotaoRail({ view, ativo, onClick, badge, somenteIcone }) {
  ...
  return React.createElement('button', {
    ...
    style: {
      ...
      color: filled ? (T.primaryHover || T.primary) : T.railText,
      border: 'none',
      cursor: 'pointer',
      position: 'relative',
...
export function RailNavegacao({ view, onChange, isAdmin, verRelatorios, notifCount, breakpoint }) {
  ...
  return React.createElement('aside', {
    style: {
      width: somenteIcone ? 68 : 220, minWidth: somenteIcone ? 68 : 220, height: '100%',
      background: T.railBg,
    ...
    breakpoint === 'desktop' && React.createElement('button', {
      ...
      style: {
        ...
        justifyContent: somenteIcone ? 'center' : 'flex-start', gap: 12,
        padding: somenteIcone ? 0 : '0 12px', border: 'none', borderRadius: T.radius,
        background: 'transparent', color: T.railText, cursor: 'pointer', marginBottom: 4,
      },
    },
      recolhida ? React.createElement(PanelLeftOpen, { size: 19 }) : React.createElement(PanelLeftClose, { size: 19 }),
      !somenteIcone && React.createElement('span', { style: { fontSize: 13, fontWeight: 600 } }, 'Recolher menu'),
    ),
