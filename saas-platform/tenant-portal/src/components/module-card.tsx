...
export default function ModuleCard({
  ...
  return (
    <button
      ...
      disabled={blocked}
      className={`group w-full min-w-0 text-left bg-surface-container-lowest rounded-xl border border-outline-variant overflow-hidden shadow-sm transition-all duration-200 flex flex-col h-full ${
    ...
    >
      ...
      <div className="flex items-center gap-3 p-3 pr-11 sm:hidden">
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${visual.gradient}`}>
          <Icon size={18} className="text-white" />
        </span>
        <div className="min-w-0 flex-1 overflow-hidden">
          <div className="flex min-w-0 items-center gap-1.5">
            <h3 className="truncate text-sm font-bold leading-tight text-on-surface">{name}</h3>
            {requires_review && <AlertTriangle size={12} className="shrink-0 text-warning" />}
            {hasNews && !requires_review && <Sparkles size={12} className="shrink-0 text-primary-600" />}
          </div>
        ...
        {opening ? (
          <Loader2 size={18} className="shrink-0 animate-spin text-primary-700" />
        ) : blocked ? (
          <Lock size={16} className="shrink-0 text-on-surface-variant" />
        ) : (
          <ChevronRight size={18} className="shrink-0 text-primary-700" />
        )}
      </div>

      {footer && <div className="border-t border-gray-100 px-3 py-2 text-sm sm:hidden">{footer}</div>}

      {/* Faixa colorida */}
      <div className={`relative hidden h-24 bg-gradient-to-br sm:block ${visual.gradient}`}>
        <div
      ...
      <div className="hidden flex-1 flex-col p-5 pt-8 sm:flex">
