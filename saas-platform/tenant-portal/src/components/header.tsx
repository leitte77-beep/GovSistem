...
import { Check, ChevronDown, LogOut, User, KeyRound, Menu } from "lucide-react";
...
export function Header() {
  ...
  const { open: menuOpen, toggle: toggleMenu } = useSidebar();
  const user = ctx?.user;
  ...
  return (
    <header
      className="fixed right-0 top-0 left-0 z-30 flex h-14 items-center justify-between gap-2 border-b border-outline-variant bg-surface-container-lowest px-4 shadow-sm sm:px-6 lg:left-64 lg:shadow-none"
    >
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-6">
        <button
          ...
          className="-ml-1 shrink-0 rounded-lg p-2 text-on-surface-variant transition hover:bg-surface-container lg:hidden"
        >
          <Menu size={22} />
        </button>
        {multiTenant && (
        ...
        {!multiTenant && (
          <span className="truncate text-sm font-bold text-on-surface sm:text-lg">{org?.name}</span>
        )}
      ...
      <div className="flex shrink-0 items-center gap-3">
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-2.5 rounded-full py-1.5 pl-2 pr-2 transition hover:bg-surface-container sm:pr-3"
            title="Meu perfil"
          >
          ...
          {profileOpen && (
            <div className="absolute right-0 z-20 mt-2 w-[min(20rem,calc(100vw-2rem))] rounded-xl border border-outline-variant bg-surface-container-lowest shadow-lg">
