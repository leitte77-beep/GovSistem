/**
 * DTOs de pessoas, famílias e busca — espelham o backend
 * (`app/schemas/people.py`). CPF/NIS sempre mascarados em listagens (LGPD);
 * a revelação sob demanda é feita por endpoint dedicado e auditado.
 * Regra: tipos de INPUT (Create/Update) têm campos opcionais; tipos de OUTPUT
 * (Out/ListItem) refletem o que o backend serializa (null quando vazio).
 */
...
export type PersonUpdate = Partial<PersonCreate> & { is_falecido?: boolean | null };
...
export type PersonListItem = {
  ...
  unidade?: string | null;
  membro_pbf?: boolean;
  beneficiario_bpc?: boolean;
  cpf_irregular?: boolean;
  /** CPF/NIS completos NÃO são expostos pela API; campos mantidos para
   *  enriquecimento local pós-revelação. */
  cpf?: string;
  nis?: string;
};
...
/** Resultado da busca para agendamento (`GET /persons/search`). */
...
export type UnifiedSearchItem = {
  ...
  nis_mascarado?: string | null;
  data_nascimento?: string | null;
  familias: Array<{ family_id: string; codigo: number; territorio?: string | null }>;
};
...
export type FamilyUpdate = Partial<FamilyCreate> & { responsavel_id?: string | null };
