...
from app.core.auth import get_current_user, get_tenant_id, require_roles
...
@router.get("/dashboard/recommendation-scope", response_model=RecommendationScopeOut)
async def dashboard_recommendation_scope(
    db: AsyncSession = Depends(get_db),
    tenant_id: uuid.UUID = Depends(get_tenant_id),
    user: User = Depends(get_current_user),
):
    """Contadores das recomendacoes do inicio.

    Sem require_roles: o bloco de recomendacoes aparece na tela inicial de
    todos os perfis, e sao apenas contagens agregadas do proprio tenant.
    """
    return await get_recommendation_scope(db, tenant_id)
