...
async def listar_pessoas(
    ...
    if pessoas:
        vinculos = (
            (
                await db.execute(
                    select(PersonFamilyMembership)
                    .where(
                        PersonFamilyMembership.person_id.in_([p.id for p in pessoas]),
        ...
        familias_ids = {v.family_id for v in vinculos}
        familias = {}
        if familias_ids:
            familias_lista = (
                (
                    await db.execute(
                        select(Family).options(selectinload(Family.responsavel)).where(
                            Family.id.in_(familias_ids),
            ...
            familias = {f.id: f for f in familias_lista}
        vinculos_por_pessoa = {v.person_id: (v, familias.get(v.family_id)) for v in vinculos}

    resultado = []
    for p in pessoas:
        vinculo, familia = vinculos_por_pessoa.get(p.id, (None, None))
        documentos = p.documentos or {}
        resultado.append({
            "id": p.id,
            "nome_exibicao": p.nome_exibicao,
            "nome_civil": p.nome_civil,
            ...
            "created_at": p.created_at,
            "is_falecido": p.is_falecido,
            "family_id": familia.id if familia else None,
            "familia_codigo": familia.codigo if familia else None,
            "familia_nome": familia.responsavel.nome_exibicao if familia and familia.responsavel else None,
            "parentesco": vinculo.parentesco if vinculo else None,
            "is_responsavel": bool(familia and familia.responsavel_id == p.id),
            "bairro": familia.bairro if familia else None,
            "unidade": familia.territorio if familia else None,
            "membro_pbf": bool(familia and familia.beneficiaria_pbf),
            "beneficiario_bpc": bool(documentos.get("beneficiario_bpc")),
            "cpf_irregular": not bool(p.cpf) or bool(documentos.get("cpf_irregular")),
        })
    return resultado


@router.get("/{person_id}/reveal/{campo}")
async def revelar_documento_pessoa(
    person_id: uuid.UUID,
    campo: Literal["cpf", "nis"],
    request: Request,
    db: AsyncSession = Depends(get_db),
    tenant_id: uuid.UUID = Depends(get_tenant_id),
    user: User = Depends(_MANAGE),
):
    """Revela um documento sob demanda, sem expô-lo nas listagens."""
    ...
    await record_audit(
        ...
        entity=f"person.{campo}",
        entity_id=person.id,
        actor=user,
        client_info=get_client_info(request),
        diff_summary={"campo": campo, "motivo": "revelacao_sob_demanda"},
    )
    await db.commit()
    return {"value": getattr(person, campo) or ""}
