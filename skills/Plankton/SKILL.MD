...
Stop 钩子 — stop_config_guardian.sh 在会话结束时通过 git diff 检测配置更改
...
推荐组合
将 ECC 安装为你的插件（代理、技能、命令、规则）
添加 Plankton 钩子以实现编写时质量强制执行
使用 AgentShield 进行安全审计
在 PR 之前使用 ECC 的 verification-loop 作为最后一道关卡
避免钩子冲突
