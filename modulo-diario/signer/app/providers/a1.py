...
class PfxA1SignerProvider(SignatureProvider):
    ...
    def sign(
        ...
        # signature time-verifiable beyond the local clock. The bridge captures
        # the exact token/gen_time so the API can persist an auditable record.
        timestamper = None
        if settings.TSA_URL:
            ...
            tsa_provider = Rfc3161TimestampProvider(settings.TSA_URL)
            timestamper = CapturingTimeStamper(
                tsa_provider, policy=settings.TSA_POLICY_OID
            )
        ...
        ts_result = getattr(timestamper, "last_result", None) if timestamper else None
        return SignedDocument(
            ...
            timestamped=ts_result is not None,
            timestamp_serial=ts_result.serial_number if ts_result else "",
            timestamp_token_base64=(
                base64.b64encode(ts_result.token).decode("ascii")
                if ts_result and ts_result.token
                else ""
            ),
            timestamp_gen_time=ts_result.gen_time if ts_result else "",
            timestamp_policy_oid=ts_result.policy_oid if ts_result else "",
            timestamp_tsa_url=settings.TSA_URL if ts_result else "",
        )
