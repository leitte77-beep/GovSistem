import type { Config } from "tailwindcss";

const config: Config = {
  content: [
    "./src/pages/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/components/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/app/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  theme: {
    extend: {
      colors: {
        "surface-dim": "#d8dadc",
        "on-primary-container": "#7493c2",
        "surface-variant": "#e0e3e5",
        "inverse-on-surface": "#eff1f3",
        error: "#ba1a1a",
        "surface-container-lowest": "#ffffff",
        "on-tertiary": "#ffffff",
        "error-container": "#ffdad6",
        "on-background": "#191c1e",
        "on-secondary-container": "#007240",
        secondary: "#006d3d",
        "secondary-container": "#8cf5b1",
        "inverse-primary": "#a8c8fa",
        background: "#f7f9fb",
        outline: "#73777f",
        "on-primary": "#ffffff",
        "on-tertiary-fixed": "#001b3d",
        primary: "#001631",
        "on-tertiary-fixed-variant": "#00468c",
        "surface-tint": "#40608b",
        "on-surface-variant": "#43474f",
        "tertiary-fixed-dim": "#a9c7ff",
        "outline-variant": "#c3c6d0",
        "on-primary-fixed-variant": "#264872",
        tertiary: "#001633",
        "surface-bright": "#f7f9fb",
        "primary-container": "#002b54",
        "secondary-fixed-dim": "#73db9a",
        "primary-fixed-dim": "#a8c8fa",
        "surface-container-highest": "#e0e3e5",
        "on-error": "#ffffff",
        surface: "#f7f9fb",
        "tertiary-fixed": "#d6e3ff",
        "tertiary-container": "#002a58",
        "on-secondary-fixed-variant": "#00522d",
        "secondary-fixed": "#8ff8b4",
        "on-secondary": "#ffffff",
        "inverse-surface": "#2d3133",
        "surface-container-low": "#f2f4f6",
        "on-tertiary-container": "#5392ef",
        "on-error-container": "#93000a",
        "surface-container": "#eceef0",
        "on-primary-fixed": "#001c3a",
        "on-secondary-fixed": "#00210f",
        "on-surface": "#191c1e",
        "primary-fixed": "#d4e3ff",
        "surface-container-high": "#e6e8ea",
        slate: {
          850: "#1a2332",
        },
        "status-published-bg": "rgba(0, 109, 61, 0.12)",
        "status-published-text": "#006d3d",
        "status-draft-bg": "rgba(67, 71, 79, 0.10)",
        "status-draft-text": "#43474f",
        "status-review-bg": "rgba(160, 92, 0, 0.12)",
        "status-review-text": "#8a4d00",
        "status-archived-bg": "rgba(71, 71, 82, 0.10)",
        "status-archived-text": "#474752",
        "status-rejected-bg": "rgba(186, 26, 26, 0.12)",
        "status-rejected-text": "#ba1a1a",
      },
      borderRadius: {
        DEFAULT: "0.125rem",
        lg: "0.25rem",
        xl: "0.5rem",
        "2xl": "1rem",
        full: "0.75rem",
      },
      spacing: {
        "stack-sm": "0.5rem",
        "stack-lg": "3rem",
        gutter: "1.5rem",
        "stack-md": "1.5rem",
        "margin-mobile": "1rem",
        "container-max": "1200px",
      },
      fontFamily: {
        "body-lg": ["Inter"],
        "headline-md": ["Inter"],
        "body-sm": ["Inter"],
        "body-md": ["Inter"],
        "label-md": ["Inter"],
        "headline-lg": ["Inter"],
        "headline-lg-mobile": ["Inter"],
        "headline-sm": ["Inter"],
        "display-lg": ["Inter"],
        display: ["Inter"],
      },
      fontSize: {
        "body-lg": ["16px", { lineHeight: "24px", fontWeight: "400" }],
        "headline-md": ["20px", { lineHeight: "28px", fontWeight: "600" }],
        "body-sm": ["14px", { lineHeight: "20px", fontWeight: "400" }],
        "body-md": ["14px", { lineHeight: "20px", fontWeight: "400" }],
        "label-md": ["12px", { lineHeight: "16px", letterSpacing: "0.05em", fontWeight: "600" }],
        "headline-lg": ["28px", { lineHeight: "36px", letterSpacing: "-0.01em", fontWeight: "700" }],
        "headline-lg-mobile": ["24px", { lineHeight: "32px", fontWeight: "700" }],
        "headline-sm": ["20px", { lineHeight: "28px", fontWeight: "600" }],
        "display-lg": ["48px", { lineHeight: "56px", letterSpacing: "-0.02em", fontWeight: "700" }],
        display: ["28px", { lineHeight: "36px", letterSpacing: "-0.02em", fontWeight: "700" }],
      },
    },
  },
  plugins: [],
};

export default config;
