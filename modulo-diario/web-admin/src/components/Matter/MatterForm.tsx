...
export default function MatterForm({ matter, isNew, initialStep }: MatterFormProps) {
  ...
  /** Interpreta o conteúdo colado (estrutura + tipo/número/ano/súmula). */
  const handleAnalyzeContent = async () => {
    const plainish = contentHtml ? contentHtml.replace(/<[^>]*>/g, " ").trim() : "";
    if (plainish.length < 10) {
      toast.error("Cole o conteúdo do ato no editor antes de interpretar");
      return;
    }
    setAnalyzing(true);
    try {
      const result = await api.analyzeMatter({ html: contentHtml, title, summary });
      const s = result.suggestions;
      if (s.summary && !summary) setSummary(s.summary);
      if (s.act_number && !actNumber) setActNumber(s.act_number);
      if (s.act_year && !actYear) setActYear(s.act_year);
      if (s.title && !title) setTitle(s.title);
      const blocks = result.document.blocks?.length ?? 0;
      toast.success(
        `Interpretado: ${blocks} bloco(s)${s.act_type_name ? ` · ${s.act_type_name}` : ""}`
      );
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Não foi possível interpretar o conteúdo");
    } finally {
      setAnalyzing(false);
    }
  };
            ...
            <div className="lg:col-span-2 space-y-gutter">
              <div className="bg-surface-bright rounded-2xl p-8 border border-outline-variant shadow-sm">
                ...
                <div className="space-y-6">
                  ...
                  {/* Interpretação automática da colagem */}
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      type="button"
                      onClick={handleAnalyzeContent}
                      disabled={!isEditable || analyzing}
                      className="inline-flex items-center gap-1 rounded-lg border border-primary px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary-fixed/20 disabled:opacity-50"
                    >
                      <span className="material-symbols-outlined text-sm">
                        {analyzing ? "progress_activity" : "auto_awesome"}
                      </span>
                      {analyzing ? "Interpretando…" : "Interpretar conteúdo colado"}
                    </button>
                    <span className="text-[10px] text-on-surface-variant">
                      Reconhece tipo, número, ano e súmula automaticamente.
                    </span>
                  </div>
                </div>
              </div>
