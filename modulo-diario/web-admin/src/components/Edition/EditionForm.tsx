...
export default function EditionForm({ edition, isNew }: EditionFormProps) {
  ...
  const [validation, setValidation] = useState<EditionValidation | null>(null);
  const [showValidation, setShowValidation] = useState(false);
  const [inspection, setInspection] = useState<EditionInspectionReport | null>(null);
  ...
  const currentEditionId = () => editionId || edition?.id || "";

  const handleAutoFill = async () => {
    const id = currentEditionId();
    if (!id) {
      toast.error("Salve a edição antes de preencher a partir da fila");
      return;
    }
    setAutoBusy(true);
    try {
      const e = await api.autoFillEdition(id);
      setItems(e.items);
      toast.success(`Fila aplicada: ${e.items.length} matéria(s) na edição`);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erro ao preencher a edição");
    } finally {
      setAutoBusy(false);
    }
  };
  ...
  const handleAutoOrder = async () => {
    ...
    setAutoBusy(true);
    try {
      const e = await api.autoOrderEdition(id);
      setItems(e.items);
      toast.success("Matérias reordenadas por seção/tipo");
    } catch (err) {
  ...
  const handleValidate = async () => {
    const id = currentEditionId();
    if (!id) {
      toast.error("Salve a edição antes de validar");
      return;
    }
    try {
      const result = await api.getEditionValidation(id);
  ...
  const handleGate = async () => {
    const id = currentEditionId();
    if (!id) {
      toast.error("Salve a edição antes de avaliar o gate");
      return;
    }
    try {
      const gate = await api.getPublicationGate(id);
      setValidation(gate);
      setShowValidation(true);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erro ao avaliar o gate");
    }
  };
  ...
  const handleInspect = async () => {
    const id = currentEditionId();
    if (!id) {
      toast.error("Gere o PDF antes de inspecionar");
      return;
    }
    try {
      const report = await api.inspectEdition(id);
      setInspection(report);
      setShowInspection(true);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erro ao inspecionar o PDF");
    }
  };
  ...
  const handlePreviewPdf = async () => {
    const id = currentEditionId();
    if (!id) {
      toast.error("Salve a edição antes de pré-visualizar");
      return;
    }
    try {
      const blob = await api.previewEdition(id);
      const url = URL.createObjectURL(blob);
      window.open(url, "_blank", "noopener,noreferrer");
      setTimeout(() => URL.revokeObjectURL(url), 60_000);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Erro ao gerar a prévia");
    }
  };
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto w-full">
      ...
      {step === 2 && (editionId || edition?.id) && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-6 bg-surface-container-low border border-outline-variant rounded-xl p-6 h-full flex flex-col min-h-[600px]">
            ...
            {(canAddItems || hasItems) && (
              <div className="mb-4 flex flex-wrap gap-2">
                {canAddItems && (
                  <button onClick={handleAutoFill} disabled={autoBusy}
                    className="inline-flex items-center gap-1 rounded-lg border border-primary px-3 py-1.5 text-xs font-bold text-primary hover:bg-primary/5 disabled:opacity-50">
                    <span className="material-symbols-outlined text-sm">playlist_add</span> Preencher com fila
                  </button>
                )}
                {canAddItems && (
                  <button onClick={handleAutoOrder} disabled={autoBusy}
                    className="inline-flex items-center gap-1 rounded-lg border border-outline px-3 py-1.5 text-xs font-bold text-on-surface hover:bg-surface-container disabled:opacity-50">
                    <span className="material-symbols-outlined text-sm">sort</span> Reordenar
                  </button>
                )}
                <button onClick={handleValidate}
                  className="inline-flex items-center gap-1 rounded-lg border border-outline px-3 py-1.5 text-xs font-bold text-on-surface hover:bg-surface-container">
                  <span className="material-symbols-outlined text-sm">fact_check</span> Validar
                </button>
                <button onClick={handleGate}
                  className="inline-flex items-center gap-1 rounded-lg border border-outline px-3 py-1.5 text-xs font-bold text-on-surface hover:bg-surface-container">
                  <span className="material-symbols-outlined text-sm">verified</span> Gate
                </button>
                <button onClick={handleInspect}
                  className="inline-flex items-center gap-1 rounded-lg border border-outline px-3 py-1.5 text-xs font-bold text-on-surface hover:bg-surface-container">
                  <span className="material-symbols-outlined text-sm">rule</span> Inspeção
                </button>
                <button onClick={handlePreviewPdf}
                  className="inline-flex items-center gap-1 rounded-lg border border-outline px-3 py-1.5 text-xs font-bold text-on-surface hover:bg-surface-container">
                  <span className="material-symbols-outlined text-sm">picture_as_pdf</span> Prévia PDF
                </button>
              </div>
            )}
      ...
      {showValidation && validation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label="Validação da edição">
          <div className="w-full max-w-lg rounded-2xl bg-surface-container-lowest p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-headline-sm font-bold text-primary">Validação da edição</h3>
              <button onClick={() => setShowValidation(false)} aria-label="Fechar" className="rounded-full p-1 hover:bg-surface-container">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <div className={clsx("mb-4 rounded-xl px-4 py-3 text-sm font-bold",
              validation.ok ? "bg-primary/10 text-primary" : "bg-error/10 text-error")}>
              {validation.ok
                ? "Pronta para publicação."
                : "Publicação bloqueada: corrija os erros abaixo."}
            </div>
            <ul className="space-y-2">
              {validation.checks.map((c) => (
                <li key={c.code} className="flex items-start gap-3 rounded-lg border border-outline-variant p-3">
                  <span className={clsx("material-symbols-outlined text-[20px]",
                    c.status === "ok" ? "text-primary" : c.status === "warning" ? "text-amber-600" : "text-error")}>
                    {c.status === "ok" ? "check_circle" : c.status === "warning" ? "warning" : "error"}
                  </span>
                  <div>
                    <p className="text-sm font-semibold">{c.label}</p>
                    {c.detail && <p className="text-xs text-on-surface-variant">{c.detail}</p>}
                  </div>
                </li>
              ))}
            </ul>
      ...
      {showInspection && inspection && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label="Inspeção do PDF">
          <div className="max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-surface-container-lowest p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-headline-sm font-bold text-primary">Inspeção do PDF</h3>
              <button onClick={() => setShowInspection(false)} aria-label="Fechar" className="rounded-full p-1 hover:bg-surface-container">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <div className={clsx("mb-4 rounded-xl px-4 py-3 text-sm font-bold",
              inspection.ok ? "bg-primary/10 text-primary" : "bg-error/10 text-error")}>
              {inspection.ok ? "PDF íntegro." : "PDF com falhas de integridade."}
            </div>
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div><dt className="text-on-surface-variant">Páginas</dt><dd className="font-semibold">{inspection.page_count}</dd></div>
              <div><dt className="text-on-surface-variant">Assinatura</dt><dd className="font-semibold">{inspection.has_signature ? `presente (${inspection.signature_count})` : "ausente"}</dd></div>
              <div><dt className="text-on-surface-variant">Matérias com página</dt><dd className="font-semibold">{Object.keys(inspection.page_number_by_anchor).length}</dd></div>
              <div><dt className="text-on-surface-variant">Tamanhos</dt><dd className="font-semibold">{inspection.page_sizes.join(", ") || "—"}</dd></div>
            </dl>
            {inspection.missing_anchors.length > 0 && (
              <p className="mt-3 text-sm text-error">
                Sem âncora: {inspection.missing_anchors.join(", ")}
              </p>
            )}
            {inspection.fonts_not_embedded.length > 0 && (
              <p className="mt-2 text-sm text-amber-700">
                Fontes não incorporadas: {inspection.fonts_not_embedded.join(", ")}
              </p>
            )}
            <ul className="mt-3 space-y-2">
              {[...inspection.errors, ...inspection.warnings].map((c, i) => (
                <li key={`${c.code}-${i}`} className="rounded-lg border border-outline-variant p-3 text-sm">
                  <span className={clsx("font-semibold", c.severity === "error" ? "text-error" : "text-amber-700")}>
                    {c.code}
                  </span>
                  <span className="ml-2 text-on-surface-variant">{c.message}</span>
                </li>
              ))}
            </ul>
          </div>
