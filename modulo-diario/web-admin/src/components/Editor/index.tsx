...
export default function Editor({
  ...
  const handleAiFormat = async () => {
    ...
    try {
      ...
      // O tipo já escolhido no formulário é a autoridade para selecionar o
      // modelo. Sem este contexto a API tenta escolher entre todos os modelos
      // da organização e pode aplicar uma portaria genérica ao pedido.
      const typeAliases: Record<string, string> = {
        "portaria": "portaria", "decreto": "decreto", "lei": "lei",
        "edital": "edital", "licitação": "licitacao", "licitacao": "licitacao",
        "contrato": "contrato", "termo": "contrato", "extrato": "extrato",
        "relatório": "relatorio", "relatorio": "relatorio", "laudo": "relatorio",
        "audiência pública": "audiencia", "audiencia publica": "audiencia",
        "resolução": "resolucao", "resolucao": "resolucao", "ofício": "oficio", "oficio": "oficio",
      };
      const normalizedType = (aiContext?.actType ?? "").toLocaleLowerCase("pt-BR");
      const documentType = typeAliases[normalizedType];
      const result = await api.aiCompose({ prompt, document_type: documentType });
