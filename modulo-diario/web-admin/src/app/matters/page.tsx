...
import { Archive, ArrowDown, ArrowUp, ArrowUpDown, ChevronDown, ChevronLeft, ChevronRight, Eye, FilePenLine, MoreHorizontal, Paperclip, Plus, Trash2, X } from "lucide-react";
...
/** Filtro inicial vindo da URL (?status=review) — os atalhos da visão geral
 *  levam direto à fila correspondente. Lido do próprio location para não
 *  exigir um limite de Suspense nesta página. */
function initialStatus(): string {
  if (typeof window === "undefined") return "";
  const value = new URLSearchParams(window.location.search).get("status") || "";
  return value in MATTER_STATUSES ? value : "";
}
...
export default function MattersPage() {
  ...
  const [stats, setStats] = useState({ total: 0, published: 0, draft: 0, review: 0, rejected: 0 });
  ...
  useEffect(() => {
    api.listMatterStats()
      .then((s) => setStats({ total: s.total, published: s.published, draft: s.draft, review: s.review, rejected: s.rejected }))
  ...
  const fetch = useCallback(() => {
    ...
    Promise.all([
      api.listMatters({ ...filters, sort, order, skip: page * PAGE_SIZE, limit: PAGE_SIZE + 1 }),
      api.countMatters(filters),
    ])
      .then(([data, count]) => {
  ...
  }, [search, status, actType, orgUnit, page, sort, order]);
  ...
  useEffect(() => { setPage(0); }, [search, status, actType, orgUnit, sort, order]);

  const pageCount = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const rangeFrom = total === 0 ? 0 : page * PAGE_SIZE + 1;
  const rangeTo = page * PAGE_SIZE + matters.length;

  const toggleSort = (field: "updated_at" | "title") => {
    if (sort === field) setOrder((o) => (o === "asc" ? "desc" : "asc"));
    else { setSort(field); setOrder(field === "title" ? "asc" : "desc"); }
  };

  const sortIcon = (field: "updated_at" | "title") => {
    if (sort !== field) return <ArrowUpDown size={13} aria-hidden="true" />;
    return order === "asc" ? <ArrowUp size={13} aria-hidden="true" /> : <ArrowDown size={13} aria-hidden="true" />;
  };
          ...
          <>
              ...
              <p className="text-sm text-on-surface-variant" aria-live="polite">
                {loading
                  ...
                  : total === 0
                    ...
                    : `Mostrando ${rangeFrom}–${rangeTo} de ${total} ${total === 1 ? "matéria" : "matérias"}`}
              </p>
              <label className="flex items-center gap-2 text-sm text-on-surface-variant lg:hidden">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSort(s as "updated_at" | "created_at" | "title");
                    setOrder(o as "asc" | "desc");
                  }}
                  className="h-9 cursor-pointer rounded-lg border border-outline-variant bg-surface px-2 text-sm font-medium text-on-surface focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15"
                >
              ...
              <table className="w-full table-fixed border-collapse text-left">
                ...
                <thead>
                  <tr className="border-b border-outline-variant bg-surface-container-low">
                    <th scope="col" aria-sort={sort === "title" ? (order === "asc" ? "ascending" : "descending") : "none"} className="w-[38%] rounded-tl-2xl px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-on-surface-variant">
                      <button type="button" onClick={() => toggleSort("title")} className="inline-flex items-center gap-1.5 rounded uppercase tracking-wider transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
                        Matéria {sortIcon("title")}
                      </button>
                    </th>
                    ...
                    <th scope="col" aria-sort={sort === "updated_at" ? (order === "asc" ? "ascending" : "descending") : "none"} className="w-[14%] px-4 py-3.5 text-xs font-bold uppercase tracking-wider text-on-surface-variant">
                      <button type="button" onClick={() => toggleSort("updated_at")} className="inline-flex items-center gap-1.5 rounded uppercase tracking-wider transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
                        Atualização {sortIcon("updated_at")}
                      </button>
                    </th>
            ...
            {!loading && !error && matters.length > 0 && (
              <div className="flex items-center justify-between rounded-b-2xl border-t border-outline-variant bg-surface-container-low px-4 py-3 sm:px-5">
                <span className="text-sm font-medium text-on-surface-variant">Página {page + 1} de {pageCount}</span>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={!hasMore || page + 1 >= pageCount}
