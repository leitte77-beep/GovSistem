...
/**
 * Canonical public URL for a matter: the persisted slug when available, with
 * the UUID as a defensive fallback (the API resolves both).
 */
export function matterHref(matter: { id?: string | null; slug?: string | null }): string {
  const key = (matter.slug || "").trim() || matter.id || "";
  return `/materias/${key}`;
}
