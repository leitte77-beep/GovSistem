...
import { matterKind, kindCounts, sectionCounts, slugify, matterHref } from "../edition-catalog";
...
describe("matterHref", () => {
  it("prefers the canonical slug and falls back to the UUID", () => {
    expect(matterHref({ id: "uuid-1", slug: "decreto-214-2026" })).toBe("/materias/decreto-214-2026");
    expect(matterHref({ id: "uuid-1", slug: null })).toBe("/materias/uuid-1");
    expect(matterHref({ id: "uuid-1", slug: "   " })).toBe("/materias/uuid-1");
  });
});
