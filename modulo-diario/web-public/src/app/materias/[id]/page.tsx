...
import type { PublicMatterPayload } from "@/lib/server/edition-loader";
...
type PublicRelation = NonNullable<PublicMatterPayload["relations"]>[number];

function otherMatter(rel: PublicRelation): { href: string; title: string } {
  const outgoing = rel.direction === "outgoing";
  const title = (outgoing ? rel.target_title : rel.source_title) || "publicação relacionada";
  const slug = ((outgoing ? rel.target_slug : rel.source_slug) || "").trim();
  const id = outgoing ? rel.target_matter_id : rel.source_matter_id;
  return { href: `/materias/${slug || id}`, title };
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = (matter.slug || "").trim() || id;
  const url = `${origin}/materias/${canonical}`;
  return {
    title,
...
export default async function MatterDetailPage({ params }: PageProps) {
  ...
  const canonical = (matter.slug || "").trim() || id;
  if (canonical !== id) permanentRedirect(`/materias/${canonical}`);
  ...
  const verificationUrl = edition?.verification_code
    ...
    : null;
  const pageUrl = `${origin}/materias/${canonical}`;
  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-gutter py-8">
        ...
        {(matter.relations?.length ?? 0) > 0 && (
          <div
            className="mb-6 rounded-xl border border-amber-300 bg-amber-50 p-4 text-body-sm text-amber-900"
            role="note"
          >
            <div className="flex items-start gap-3">
              <span aria-hidden="true" className="material-symbols-outlined mt-0.5 text-amber-700">
                history_edu
              </span>
              <div className="space-y-1">
                {(matter.relations ?? []).map((r) => {
                  const other = otherMatter(r);
                  return (
                    <p key={r.id}>
