...
- [x] Conectado ao fluxo: quando `TSA_URL` está configurada, o `PdfSigner` usa um `CapturingTimeStamper` (bridge sobre o `Rfc3161TimestampProvider`) e o token RFC 3161 é embutido no CMS. O `SignResponse` devolve `timestamp_status`, `timestamp_serial`, `timestamp_token_base64`, `timestamp_gen_time` e `timestamp_policy_oid`; a API persiste `gen_time` + token em `TimestampRecord` (status `present`, `pending_validation`) para auditoria.
- [ ] Configurar ACT ICP-Brasil real e validar a cadeia do token de ponta a ponta (a validação independente da cadeia/ACT continua pendente; por honestidade o estado é `timestamped=true` com `trusted` apenas quando a cadeia ICP-Brasil é validada).
...
| Variável | Efeito |
|---|---|
| `TSA_URL` | ACT RFC 3161 usada no PAdES (token embutido no CMS e persistido em `TimestampRecord`). |
| `TSA_POLICY_OID` | Enviada no `req_policy` da requisição RFC 3161 (bridge `CapturingTimeStamper`); quando o token ecoa a política, ela é registrada. |
