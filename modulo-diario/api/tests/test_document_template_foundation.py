...
async def test_training_file_accepts_utf8_txt(api_client, ctx):
    client = api_client(ctx.admin)
    model_id = await _create_model(client, "treino-txt")
    resp = await client.post(
        f"/api/v1/document-models/{model_id}/training-files",
        files={"file": ("03-portaria-secretaria.txt", "PORTARIA Nº 057/2025/SMS\n\nRESOLVE:\n".encode("utf-8"), "text/plain")},
    )
    assert resp.status_code == 201
    assert resp.json()["has_text"] is True
...
def test_learning_prompt_limits_repeated_table_rows():
    ...
    assert "cada tipo de ato" in prompt
    assert "leis e decretos" in prompt
    assert "editais e licitações" in prompt


def test_learning_prompt_preserves_large_pdf_coverage():
    text = "INÍCIO\n" + ("meio " * MAX_CHARS_PER_DOC) + "\nFIM DA EDIÇÃO"
    prompt = build_user_message("portaria", [("edicao.pdf", text)])
    assert "INÍCIO" in prompt
    assert "FIM DA EDIÇÃO" in prompt
    assert "trecho intermediário preservado" in prompt


def test_visual_profile_handles_plain_text_without_inventing_style():
    assert extract_visual_profile("modelo.txt", b"PORTARIA\nRESOLVE:") == ""
