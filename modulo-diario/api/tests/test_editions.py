...
@patch("httpx.AsyncClient")
@pytest.mark.anyio
async def test_sign_edition_persists_rfc3161_timestamp_record(
    mock_http, mock_storage, mock_read_pdf,
    mock_capture, mock_audit, client, override_db_and_auth,
):
    """O token/gen_time devolvidos pelo signer viram um TimestampRecord auditável."""
    ...
    mock_resp.json.return_value = {
        ...
        "timestamp_serial": "999",
        "timestamp_token_base64": "dG9rZW4=",
        "timestamp_gen_time": "2026-09-03T12:00:00+00:00",
        "timestamp_policy_oid": "2.16.76.1.7.1.11.1",
        "timestamp_tsa_url": "https://tsa.exemplo.br",
    }
    mock_http_client = AsyncMock()
    mock_http_client.__aenter__.return_value = mock_http_client
    mock_http_client.__aexit__.return_value = None
    ...
    records = [
        ...
        if isinstance(call.args[0], TimestampRecord)
    ]
    assert len(records) == 1
    record = records[0]
    assert record.gen_time == datetime.fromisoformat("2026-09-03T12:00:00+00:00")
    assert record.token == "dG9rZW4="
    assert record.serial_number == "999"
    assert record.policy_oid == "2.16.76.1.7.1.11.1"
    assert record.tsa_url == "https://tsa.exemplo.br"
    assert record.validation_status == "pending_validation"
