...
@pytest.mark.asyncio
async def test_relation_public_exposes_canonical_slugs(db_session):
    """A relação pública leva os slugs de origem/destino para linkar a matéria."""
    org, u, _, mk = await _seed(db_session, "org-slug-rel", "sr@x", "S")
    a = mk("7")
    b = mk("8")
    await _flush_all(db_session)
    await db_session.refresh(a)
    ...
    rels = await list_relations_for_matter(db_session, org.id, a.id)
    public = [relation_public(r) for r in rels["outgoing"]]
    assert public[0]["source_slug"] == "portaria-s-7"
    assert public[0]["target_slug"] == "portaria-s-8"
    assert public[0]["relation_type"] == "rectifies"
