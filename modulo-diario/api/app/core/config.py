...
class Settings(BaseSettings):
    ...
    # Empty means "use the migration head shipped in this code revision".
    ALEMBIC_EXPECTED_HEAD: str = ""
    ...
    # Independent RFC 3161 validation. Empty trust store means the TSA chain is
    # reported as "não verificada" (indeterminate), never as valid.
    TSA_URL: str = ""
    TSA_TRUST_ROOTS_PATH: str = ""          # trust anchors only (e.g. ICP-Brasil roots)
    TSA_INTERMEDIATES_PATH: str = ""        # auxiliary CAs used to build the path
    TSA_CRL_PATH: str = ""                  # local CRLs consumed offline
    TSA_POLICY_OID: str = ""                # required time-stamp policy (reqPolicy)
    TSA_REVOCATION_MODE: str = "soft-fail"  # soft-fail | hard-fail | require
    TSA_ALLOW_FETCHING: bool = False         # fetch CRL/OCSP online
    TSA_REQUIRED_FOR_PUBLICATION: bool = False
    ...
    # Editorial keep rules for the edition PDF: minimum lines kept together at
    # the bottom (before a break) and at the top (after a break).
    PDF_MIN_LINES_BEFORE_BREAK: int = 2
    PDF_MIN_LINES_AFTER_BREAK: int = 2
