...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # Optional editorial section override for auto-assembled editions. When
    # empty the section is inferred from the act type name (classification).
    "section": None,
    "dynamic_fields": [],
}
...
def normalize_config(raw: dict | None) -> dict:
    ...
    if section is not None and str(section).strip():
        if len(str(section).strip()) > 120:
            raise ActTypeConfigError("Seção editorial deve ter no máximo 120 caracteres.")
        cfg["section"] = str(section).strip()
    else:
        cfg["section"] = None
