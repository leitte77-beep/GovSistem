...
def build_search_blob(matter: Matter) -> str:
    """Searchable text for a matter: body + act identification + dynamic fields.

    Includes the ementa (summary), act number/year and per-act-type dynamic
    values (processo, licitação, contrato, empresa, CNPJ, ...) so citizens can
    find an act by these terms, not only by title/body.
    """
    parts = [
        getattr(matter, "title", None),
        getattr(matter, "summary", None),
        getattr(matter, "plain_text", None),
    ]
    act_number = getattr(matter, "act_number", None)
    if act_number:
        act_type_name = matter.act_type.name if getattr(matter, "act_type", None) else ""
        year = getattr(matter, "act_year", None)
        parts.append(f"{act_type_name} {act_number}" + (f"/{year}" if year else ""))
    metadata = getattr(matter, "metadata_json", None)
    if isinstance(metadata, dict):
        parts.extend(metadata.values())
    return " \n".join(str(p) for p in parts if p not in (None, "", [], {})).strip()
...
class PostgresFtsProvider(SearchProvider):
    ...
    async def index_matter(self, matter: Matter, edition: Edition, db: AsyncSession) -> None:
        ...
        # The stored blob also carries act number/year, the ementa and the
        # per-act-type dynamic fields so FTS finds them.
        idx.plain_text = build_search_blob(matter)
