...
def relation_public(rel: MatterRelation) -> dict:
    ...
    Slugs let the public page link straight to the related matter without an
    extra lookup. Only relationships already loaded in this instance are read
    (never triggers a lazy load in async sessions).
    """
    from app.services.matter_slug import matter_slug

    source = rel.__dict__.get("source")
    target = rel.__dict__.get("target")
    return {
        "id": str(rel.id),
        "source_matter_id": str(rel.source_matter_id),
        "target_matter_id": str(rel.target_matter_id),
        ...
        "source_slug": matter_slug(source) if source else None,
        "target_slug": matter_slug(target) if target else None,
