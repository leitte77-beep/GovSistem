...
# Alternância de gênero: {{genero:ao servidor|à servidora}}. A chave precisa
# conter "genero" para que o marcador se leia sozinho no texto do modelo.
GENDER_MARKER_RE = re.compile(
    r"\{\{\s*([a-zA-Z0-9_.]*genero[a-zA-Z0-9_.]*)\s*:\s*([^|{}]*)\|([^|{}]*)\}\}"
)
...
class SectionSpec(BaseModel):
    ...
    template_html: str = Field(default="", max_length=100000)
    ...
    # Aparência observada no documento de referência. Declarativa e segura:
    # não contém HTML/CSS arbitrário.
    font_weight: str = "normal"
    italic: bool = False
    uppercase: bool = False
    indent_em: float = Field(default=0, ge=0, le=12)
    space_before_mm: float = Field(default=0, ge=0, le=30)
    space_after_mm: float = Field(default=0, ge=0, le=30)
    ...
    def _validate_kind(self) -> "SectionSpec":
        if self.template_html:
            ...
            self.template_html = sanitize_html(self.template_html)
        if self.font_weight not in {"normal", "bold"}:
            raise ValueError("font_weight deve ser normal ou bold.")
...
class DocumentModelConfig(BaseModel):
    ...
    def _iter_all_texts(self):
        def walk(sections: list[SectionSpec]):
            for s in sections:
                for value in (s.text, s.number, s.suffix, s.template_html):
                    if value:
                        yield value
...
def _markers(text_value: Optional[str]) -> list[str]:
    if not text_value:
        return []
    # A alternância de gênero também referencia um campo declarado, então entra
    # na mesma checagem de "todo marcador tem campo".
    gender = [m.group(1) for m in GENDER_MARKER_RE.finditer(text_value)]
    return MARKER_RE.findall(text_value) + gender
