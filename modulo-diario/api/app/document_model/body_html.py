...
def article_html(block, index: int) -> str:
    ...
    style = getattr(block, "metadata", {}).get("style", {})
    style_attr = _style_attr(style)
    parts = [f'<p class="doc-article"{style_attr}><span class="art-num">{esc(label)}</span> ']
...
def _style_attr(style: dict) -> str:
    if not style:
        return ""
    css = (
        f"text-align:{align(style.get('alignment'))};"
        f"font-weight:{'bold' if style.get('font_weight') == 'bold' else 'normal'};"
        f"font-style:{'italic' if style.get('italic') else 'normal'};"
        f"text-transform:{'uppercase' if style.get('uppercase') else 'none'};"
        f"text-indent:{float(style.get('indent_em', 0) or 0):g}em;"
        f"margin-top:{float(style.get('space_before_mm', 0) or 0):g}mm;"
        f"margin-bottom:{float(style.get('space_after_mm', 0) or 0):g}mm;"
    )
    return f' style="{css}"'
...
def blocks_to_html(document) -> str:
    ...
    for block in document.blocks:
        if block.metadata.get("template_html"):
            ...
            out.append(sanitize_html(block.metadata["template_html"]))
            continue
        style = getattr(block, "metadata", {}).get("style", {})
        attr = _style_attr(style)
        btype = getattr(block, "type", "")
        if btype == "heading":
            out.append(f'<h{block.level} class="doc-heading"{attr}>{esc(block.text)}</h{block.level}>')
        elif btype == "command":
            out.append(f'<p class="doc-command"{attr}>{esc(block.text)}</p>')
        elif btype == "preamble":
            out.append(f'<p class="doc-preamble"{attr}>{esc(block.content)}</p>')
        elif btype == "paragraph":
            out.append(f'<p class="doc-paragraph"{attr}>{esc(block.content)}</p>')
        elif btype == "quote":
