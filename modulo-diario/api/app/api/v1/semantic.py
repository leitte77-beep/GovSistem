...
from app.schemas.semantic import (
    MatterAnalyzeRequest,
    MatterAnalyzeResponse,
...
from app.semantic.suggestions import suggest_metadata
...
@router.post("/matters/analyze", response_model=MatterAnalyzeResponse)
async def analyze_pasted_matter(
    body: MatterAnalyzeRequest,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(require_roles("AUTOR", "REVISOR", "DIAGRAMADOR", "ADMIN")),
):
    """Interpret a raw paste (Word/HTML/plain) before the matter exists.

    Returns the structured semantic document plus deterministic suggestions of
    act type/number/year so the author confirms instead of typing. Nothing is
    persisted here: the caller reviews the result and then creates the matter.
    """
    await _require_engine(db, user)
    if not (body.html or body.plain or "").strip():
        raise HTTPException(422, "Conteúdo vazio: envie 'html' ou 'plain'.")
    ...
    return MatterAnalyzeResponse(
        document=document,
        suggestions=suggest_metadata(document, title=body.title),
