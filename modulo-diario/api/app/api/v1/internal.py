...
@router.get("/internal/health/dependencies")
async def internal_dependencies(
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_internal_key),
):
    """Full dependency diagnosis (internal/admin network only)."""
    from app.services.readiness import ReadinessService, summarize

    checks = await ReadinessService().dependency_checks(db, probe=True)
    return {
        "status": summarize(checks),
        "dependencies": {
            c.code.lower(): {"status": c.status, "detail": c.detail}
            for c in checks
        },
    }


@router.get("/internal/publication-capability")
async def internal_publication_capability(
    db: AsyncSession = Depends(get_db),
    _: None = Depends(require_internal_key),
):
    """Whether an official publication can proceed right now.

    Deliberately distinct from API readiness: the portal may be fully healthy
    while official publication is blocked (e.g. TSA unavailable).
    """
    from app.services.readiness import ReadinessService, publication_capability

    checks = await ReadinessService().publication_checks(db, probe=True)
    capability = publication_capability(checks)
    capability["checks"] = [
        {"code": c.code, "status": c.status, "detail": c.detail} for c in checks
    ]
    return capability
