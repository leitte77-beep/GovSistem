...
async def _verify_schema() -> None:
    ...
    expected_head = settings.ALEMBIC_EXPECTED_HEAD.strip()
    if not expected_head:
        # Default to the migration head shipped in this code revision, instead
        # of a hardcoded revision that silently goes stale and masks a pending
        # migration at startup.
        from alembic.config import Config
        from alembic.script import ScriptDirectory

        expected_head = ScriptDirectory.from_config(Config("alembic.ini")).get_current_head() or ""
