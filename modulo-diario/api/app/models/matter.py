...
class Matter(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "slug", name="uq_matters_org_slug"),
    )
    ...
    slug: Mapped[Optional[str]] = mapped_column(
        ...
        comment="Canonical public slug (decreto-214-2026); unique per org, "
                "frozen after publication.",
    )
    slug_locked_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True,
        comment="Permanent marker: set on first publication. Once set, the "
                "public slug can never change again (app guard + DB trigger).",
    )
    slug_locked_reason: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True,
        comment="Why the slug was locked (e.g. first_publication, backfill).",
    )
