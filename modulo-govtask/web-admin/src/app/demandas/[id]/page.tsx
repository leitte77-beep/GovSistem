...
import { ArrowLeft, CalendarDays, Clock3, Copy, FileDown, ListTodo, RefreshCw, ShieldAlert, UserRound } from "lucide-react";
...
export default function DemandaDetalhePage() {
  ...
  useEffect(() => { Promise.all([api.getDemandaV2(id), api.listarTarefasDemanda(id), api.timelineDemanda(id)]).then(([d, t, e]) => { setDemanda(d); setTarefas(t); setEventos(e.items || []); }).catch(e => setErro(e.message)); }, [id]);
