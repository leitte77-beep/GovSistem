...
import { bootstrapTokenFromQuery } from "@/lib/api";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      ...
      if (!hasTokenInUrl) {
        // Sem token SSO e sem sessão — redireciona para o login do painel admin
        window.location.href = "https://admin.govsistem.com.br/login";
      }
    }
  }, [authLoading, user]);

  // Se o bootstrap da URL carregou o usuário, a tela de login nem aparece
  if (authLoading) {
    return (
  ...
  // Se já autenticado (bootstrap funcionou), não mostra login
  if (user) return null;
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900">GovTask</h1>
          <p className="text-gray-500 text-sm mt-1">Gestão de Convênios Públicos</p>
        </div>
