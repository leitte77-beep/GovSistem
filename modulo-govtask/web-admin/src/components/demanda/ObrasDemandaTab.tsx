...
import type { DiarioObra, Obra, RegistroFoto, VistoriaObra } from "@/types/govtask";
import { VISTORIA_TIPOS, VISTORIA_STATUS } from "@/types/govtask";

type Props = { demandaId: string; podeEditar: boolean };
type SubAba = "cronograma" | "diario" | "fotos" | "vistorias";
      ...
      {obra && (
        <>
          ...
          <nav className="flex gap-1 border-b border-slate-200" aria-label="Seções da obra">
            {([["cronograma", "Cronograma"], ["diario", "Diário"], ["fotos", "Fotos"], ["vistorias", "Vistorias"]] as const).map(([key, label]) => (
