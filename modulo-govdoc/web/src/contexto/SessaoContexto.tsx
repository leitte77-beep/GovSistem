...
type Valor = {
  dados: Sessao | null; carregando: boolean;
  entrar: (email: string, senha: string) => Promise<void>;
  sair: () => Promise<void>; recarregar: () => Promise<void>;
...
export function ProvedorSessao({ children }: { children: React.ReactNode }) {
  ...
  async function entrar(email: string, senha: string) {
    ...
    // access_token por uma sessão do GovDoc (ponte /auth/dev/session).
  ...
  const valor = useMemo(
    () => ({ dados, carregando, entrar, sair, recarregar, loginSaasDev: LOGIN_SAAS_DEV }),
