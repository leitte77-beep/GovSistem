/**
 * Cliente HTTP central do GovDoc.
 *
 * A URL da API nunca é fixada no código: vem de `VITE_GOVDOC_API_URL` (gravado
 * pelo resolvedor de portas). Sem a variável, usa caminho relativo — o que
 * funciona atrás do nginx do Docker e do proxy do Vite.
 *
 * A sessão é única: o token chega da plataforma GovSistem (login único) via
 * `?token=` na URL ou pela ponte dev. O GovDoc não emite nem renova tokens.
 */
...
// Captura o token do SSO (`?token=`) ANTES de o React montar: o React Router
// navega o catch-all `*` para /painel na primeira renderização, e essa
// navegação limpa a query string da URL — sem esta captura antecipada o token
// chegaria já "apagado" (o mesmo motivo pelo qual o ChatGov lê o token no
// inicializador do estado, não em efeito).
try {
...
/**
 * Carrega a prévia do documento como URL local (blob).
 *
 * O endpoint exige cabeçalho de autorização, que um `<iframe src>` não envia —
 * por isso o conteúdo é buscado autenticado e transformado em object URL.
 * Nunca há URL pública permanente para o arquivo.
 */
export async function carregarPrevia(documentoId: string): Promise<{ url: string; tipo: string }> {
  const resposta = (await requisitar<Response>(`/documentos/${documentoId}/visualizar`, {
    bruto: true,
