import { ArrowUpRight, FileHeart, LoaderCircle, LockKeyhole, Mail } from 'lucide-react';
...
export function Entrar() {
  ...
  if (dados) return <Navigate to="/painel" replace/>;
  ...
  if (!loginSaasDev) {
    return <main className="tela-login">
      <section className="caixa-login">
        <div className="marca-login"><FileHeart size={34}/><span>Gov<span className="destaque">Doc</span></span></div>
        <h1>Bem-vindo</h1><p className="subtitulo">O acesso à gestão documental é feito pelo GovSistem.</p>
  ...
  async function enviar(e: React.FormEvent) {
    ...
    try { await entrar(email, senha); navegar('/painel'); }
    catch (e) { setErro(e instanceof ErroApi ? e.message : (e instanceof Error ? e.message : 'Não foi possível conectar ao GovDoc.')); }
